...
void main() {
  ...
  testWidgets('AcademicServiceCard keeps a stable loading footprint', (
    WidgetTester tester,
  ) async {
    final localCompleter = Completer<List<dynamic>>();

    await pumpCard(
      tester,
      AcademicServiceCard(
        layout: LayoutTokens.compact(),
        loadCachedList: (_) => localCompleter.future,
        repository: buildRepo(),
      ),
      width: 430,
    );
    ...
    expect(cardSize.height, greaterThanOrEqualTo(220));

    localCompleter.complete(testServices);
    await pumpUntilFound(tester, find.text('Service roles'));
