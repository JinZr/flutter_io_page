...
The build script creates a WebAssembly-capable release build with source maps
and icon tree-shaking. Release output is written to the default `build/web`
folder (or `build/web-html` when using `tool/perf_measure.sh`).
...
3. Serve the release build and collect Lighthouse metrics:
   ...
   ```bash
   flutter run -d web-server --web-hostname 127.0.0.1 --web-port 8080 --release
